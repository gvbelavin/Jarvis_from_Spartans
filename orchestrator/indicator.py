...
Особенности:
- RX и TX платы работают на 115200 (если TX идёт мусором —
  попробовать другой физический USB-порт компа, дело было в порту, не в плате)
- Сразу после сброса плата плюёт вывод ROM-загрузчика ESP32 на другом baud
  (это норма), затем идут чистые строки READY / OK / EVT
