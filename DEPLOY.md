...
живёт внутри `app.py`. Код — `orchestrator/web_audio.py`, страницы — `webapp/`.

Видов два, оба ведут в один пайплайн:

| Адрес | Вид | Как работает |
|---|---|---|
| `/` | **шар** (по умолчанию) | Микрофон открыт постоянно, поток идёт на плату по WebSocket. Wake word («Джарвис») и конец фразы считает **плата** — теми же `wake_word.detect` и `CommandRecorder`, что и для железного микрофона. Экран — только светящийся шар: он меняет цвет по состоянию и шевелится в такт голосу. |
| `/classic` | старый | «Нажал — сказал — нажал». Wake word не нужен, видно распознанный текст и ответ. |

Из шара старый вид открывается через меню (тап по экрану → кнопка в
углу), из старого — по заголовку «Джарвис».

Про шар стоит знать три вещи, все — ограничения Safari, а не плату:

- **Первое касание.** Микрофон и звук браузер включает только из жеста,
  поэтому при открытии шар спит: один тап — и дальше руки не нужны.
- **Экран должен быть включён.** Со свёрнутой вкладкой или на
  заблокированном экране Safari останавливает захват. Страница просит
  Wake Lock, так что на зарядке экран не гаснет сам.
- **Трафик.** Поток 16 кГц — примерно **115 МБ в час**. По Wi-Fi
  незаметно, по мобильному интернету ощутимо.

Пока Джарвис отвечает, микрофон закрывается совсем: иначе iOS уводит звук
в разговорный динамик (ответ еле слышно), а Джарвис слышал бы сам себя.
Перебить его на полуслове нельзя — он дослушает до конца.
...
2. Safari → `https://app.heyjarvis.ru:8443` → тап по шару → ввести токен
   (меню в углу) → разрешить микрофон.
...
- **«Джарвис уже слушает другое устройство»** — поток занят: шар держит
  ровно одно соединение (wake word на плате в одном экземпляре). Закройте
  вкладку на втором телефоне.
- **Шар не реагирует на «Джарвис»** — проверьте по логу, доходит ли
  `[web] Wake word с телефона`. Если нет, микрофон телефона тише
  железного: говорите ближе или снизьте `WAKE_WORD_THRESHOLD` в
  `audio_module/config.py`.
